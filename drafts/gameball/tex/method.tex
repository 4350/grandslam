...
If the copula has time-varying dynamics, this implies that the level of diversification benefits are also time-varying. We use a measure of conditional diversification benefits from \textcite{ChristoffersenErrunzaJacobLanglois2012} to determine how modern and classic value differ in terms of diversification benefits. 
...
\end{align}
where $r_{i,t}$ are simple returns of assets $i$, to ensure that the weighted average return property of a portfolio holds, and $F_{i,t}^{-1}(p)$ is the conditional inverse CDF, which is also equivalent to the negative estimate of $p\%$ Value-at-Risk (VaR).
...
\end{align}
where $w_{i,t}$ are the dynamic portfolio weights.

From the equality in \autoref{eq:es1}, we know that the upper bound of $ES$, where there are no diversification benefits of the portfolio, is
\begin{align}
    \overline{ES}^p_t(w_t) = \sum\limits^N_{i=1} w_{i,t} ES^p_t(R_{i,t})
\end{align}
Similarly, a lower bound on the portfolio's $ES$ is
\begin{align}
    \underline{ES}^p_t(w_t) = -F^{-1}_t(w_t, p) 
\end{align}
where $F^-1_t(w_t, q)$ is the inverse CDF for the portfolio. The lower bound is therefore equivalent to the case when the portfolio's $ES$ is equivalent to the portfolio's $VaR$.
