...
marginalLogLikelihood <- function(shocks, uv.dists, cluster) {
  ...
  marginalLLFn <- function(i, shocks, uv.dists) {
    sum(dghyp(shocks[, i], uv.dists[[i]], logvalue = T))
  }
...
#' @export
jointLogLikelihood <- function(shocks, uv.dists, mv.df, mv.skew, alpha, beta, cluster = NULL) {
  ...
  # matrix
  jointLLFn <- function(t, mv.df, mv.skew, shocks, Correlation) {
