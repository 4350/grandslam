...
#' @export 
#' 
diagPlots <- function(df, factor, newsimp, empdens) {
  ...
  g <- arrangeGrob(out.ret, out.qq, out.acf, out.pacf,
                   ...
                   ncol = 2)
  ggsave(file= paste('output/GARCHDiagnostics/GARCHDiagnostics', factor, 'jpeg', sep = '.'), g, width = 8.3, height = 11.7, units = 'in', limitsize = F) #saves g
